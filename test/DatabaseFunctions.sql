...
## 8) Book Browsing
## conjunctive query with order options


SELECT Books.ISBN, title, authors, publisher, year, copies, price, format,keywords, subject, AVG(score) FROM Books natural join Feedbacks
WHERE LOWER(authors) LIKE LOWER("%authorName%")
AND LOWER(publisher) LIKE LOWER("%publisher%")
